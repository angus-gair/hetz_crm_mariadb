...
router.get('/calendar/:date', async (req: Request, res: Response) => {
  try {
    ...
    // If no meetings found using the date range, try getting all meetings and filtering
    if (meetings.length === 0) {
      console.log('[API] No meetings found with date range filter, trying to get all meetings');
      
      try {
        const allMeetings = await suiteCRMService.getAllMeetings();
        ...
        console.log(`[API] Found ${meetings.length} meetings for ${date} in the full list`);
