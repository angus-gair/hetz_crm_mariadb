...
const ContactForm = ({ apiUrl, apiToken, onSuccess, onError }) => {
  ...
  const validateForm = () => {
    ...
    if (formData.website.length > 0) {
      // Silent failure - don't inform bots they've been caught
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate form
    if (!validateForm()) {
      // If honeypot was filled (likely a bot), pretend success
      if (errors.honeypot) {
        setSubmitResult({
          status: 'success',
    ...
    try {
      ...
      const response = await axios.post(`${apiUrl}/contacts`, submissionData, {
        headers: {
          'Authorization': `Bearer ${apiToken}`,  // Still include token if available
